...
This repository is the reference starter for an independently releasable Deckhand plugin. It ships as a working, read-only `dh-example` status provider so contract, discovery, configuration, and tests are demonstrable before integration-specific behavior is added.
...
5. Replace the static provider with the smallest integration boundary that can be tested without a real site.
6. Pin the supported Deckhand core release or immutable commit and run the full conformance suite.
